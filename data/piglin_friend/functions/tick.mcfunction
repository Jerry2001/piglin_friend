...
execute as @e[tag= Jim, limit=1] at @s if entity @e[type=boat, distance= 0..8] run execute as @e[type= minecraft:boat, sort= nearest, limit=1] at @s unless entity @e[type= minecraft:vex, limit=1, distance=0..1] run summon minecraft:vex ~ ~ ~ {ActiveEffects:[{Id:14,Amplifier:0,Duration:20000000,ShowParticles:0b}], Silent: 1b, Tags:[toKill], NoAI: 1b, DeathTime: 19}

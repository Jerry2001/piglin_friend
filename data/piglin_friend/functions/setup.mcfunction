...
execute if score piglinSan piglin_friend matches 0 unless entity @e[tag=Jim] as @e[tag=leader, limit=1] at @s run summon minecraft:piglin ~ ~ ~ {IsImmuneToZombification: 1b, Tags:[Jim], PersistenceRequired: 1b, CustomName:"\"Jimmy\"", Attributes:[{Base:0d, Name:"generic.attack_damage"}], HandItems: [{id: "minecraft:poppy", Count: 1b}, {}]}
